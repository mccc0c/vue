...
return g?f(g):f;};})();return this.each(function(){a(this).append(tmpl(c,b));});},modal:function(b){var c={bgscroll:false,width:null,height:null,dragable:true,ani:true,bgclose:false,mobileDetect:true,myalert:false,alertCenter:true,hideDuration:0,top:"50%",left:"50%",overflow:false,beforeOpen:function(){},onOpen:function(){},onClose:function(){},onOk:function(){}};
var e=a.extend({},c,b);var d=this;return(function(){var k;var g=d.selector;var m=e.dialog?a(e.dialog):d;var r=e.dialog?m.selector:d.selector;var h=m.height();
...
}}a(document).on("click",g,function(t){t.stopPropagation();e.beforeOpen(a(this));i();e.onOpen(a(this));k=a(this);});function q(t,u){m.css({left:e.left,top:e.top,"margin-left":-t/2+"px","margin-top":-u/2+"px"});
...
},200);}}f.on("click",function(){if(e.bgclose){s();e.onClose();}});a(document).on("click",r+" .dclose",function(){s();e.onClose();return false;});a(document).on("click",r+" .dcancle",function(){s();
e.onClose();return false;});a(document).on("click",r+" .dokay",function(){s();e.onOk(k);return false;});a(document).on("mousedown",r+".dtitle",function(t){if(e.dragable){p=true;
}n=t.clientX-parseInt(m.css("left"));j=t.clientY-parseInt(m.css("top"));return false;});a(document).mousemove(function(B){if(p){var B=B||window.event;var v=B.clientX-n;
var x=B.clientY-j;var A=m.width()/2;var z=m.height()/2;var u=document.documentElement.clientWidth-A;var y=document.documentElement.clientHeight-z;if(v<A){v=A;
}else{if(v>u){v=u;}}if(x<z){x=z;}else{if(x>y){x=y;}}m.css({left:v+"px",top:x+"px"});return false;}});a(document).mouseup(function(t){p=false;});d.open=function(){console.log(1);
e.beforeOpen();i();e.onOpen();};d.close=function(){s();e.onClose();};return d;})();},isMobile:function(){if(/Android|Windows Phone|webOS|iPhone|iPod|BlackBerry|iPad|Opera/i.test(navigator.userAgent)){return true;
...
if(d.animate){if(l.is("."+e)){l.removeClass(e);k.slideUp();}else{l.addClass(e);k.slideDown();}if(d.collapse){j.removeClass(e);i.slideUp();}}else{if(l.is("."+e)){l.removeClass(e);
k.hide();}else{l.addClass(e);k.show();}if(d.collapse){j.removeClass(e);i.hide();}}});});},dropdown:function(b){var c={event:"mouseenter"};var d=a.extend({},c,b);
return this.each(function(){var e=a(this);e.find("ul li:has(ul)").on(d.event,function(){a(this).children("ul").stop(true,true).slideDown(400);});e.find("ul li:has(ul)").on("mouseleave",function(){a(this).children("ul").stop(true,true).slideUp("fast");
...
d.src=f;d.onload=function(){a++;if(a==c.length){b();}};});}